...
async function preloadLibrariesForPage(page) {
    if (page === "size") {
        await Promise.all([getHtml2canvas(), getQRCode()]);
        return;
    }

    if (page === "shiny") {
        await Promise.all([getQRCode(), getMermaid()]);
    }
}
...
function restoreOfficialActivityState() {
    ...
    const hiddenState = window.localStorage.getItem(
        OFFICIAL_ACTIVITY_HIDDEN_KEY,
    );
...
const masterCreaturesById = ref(new Map());
...
const shinyVisualActive = computed(
    () => forceShiny.value && canEnableShinySwitch(),
);
...
function addShinyOwned() {
    ...
    if (
        shinyFemaleOnlyPets.has(finalName) &&
        shinyOwnedDraftGender.value === "male"
    ) {
    ...
    const exists = shinyOwnedList.value.some(
        (x) => `${x.name}::${x.gender}` === key,
    );
    if (exists) {
...
function removeShinyOwned(item) {
    shinyOwnedList.value = shinyOwnedList.value.filter(
        (x) => !(x.name === item.name && x.gender === item.gender),
    );
}
...
function getMappedCreatureImageFile(petId) {
    const id = normalizePetId(petId);
    if (!id) return "";

    const creature = masterCreaturesById.value.get(id);
    if (!creature) return "";
...
function getCreatureImageUrl(petId) {
    ...
    const fileName = getMappedCreatureImageFile(petId);
    if (!fileName) return "";
    return `${baseUrl}creature-atlas/${encodeURIComponent(fileName)}`;
}

function updateShareImagesReady() {
    if (typeof document === "undefined") return;
    const imgs = Array.from(
        document.querySelectorAll(".share-poster-target img"),
    );
    if (imgs.length === 0) {
        shareImagesReady.value = true;
        return;
    }
    shareImagesReady.value = imgs.every(
...
function evaluateRow(diameter, weight, row) {
    ...
    if (dIn && wIn) {
        ...
        const precisionBoost =
            1 +
            0.16 * (1 / (1 + span(row.diameterRange) * 12)) +
            0.12 * (1 / (1 + span(row.weightRange) * 2));
    ...
    const score =
        1 /
        (1 +
            distanceToRange(diameter, row.diameterRange) / 0.05 +
            distanceToRange(weight, row.weightRange) / 1.0);
...
function extractEggMeasurementRows(eggJson) {
    ...
    for (const group of groups) {
        ...
        const rangeItems = Array.isArray(group?.rangeItems)
            ? group.rangeItems
            : [];
        const exactItems = Array.isArray(group?.exactItems)
            ? group.exactItems
            : [];
...
async function loadDataset() {
    ...
    try {
        ...
        const [eggRes, masterRes, evoRes] = await Promise.all([
            fetch(`${baseUrl}data/egg-measurements-final.json`, {
                cache: "no-store",
            }),
            fetch(`${baseUrl}data/creatures-master-list.json`, {
                cache: "no-store",
            }),
            fetch(`${baseUrl}data/evolution-chains.json`, {
                cache: "no-store",
            }),
        ]);
        ...
        if (!masterRes.ok)
            throw new Error(`master dataset HTTP ${masterRes.status}`);
        ...
        const idMap = new Map();
        const masterCreatures = Array.isArray(masterJson?.creatures)
            ? masterJson.creatures
            : [];
        masterCreaturesById.value = new Map();
        for (const c of masterCreatures) {
            idMap.set(c.name, c.id);
            const normalizedId = normalizePetId(c.id);
            if (normalizedId) masterCreaturesById.value.set(normalizedId, c);
        }
...
async function onShareLongImage() {
    ...
    try {
        ...
        const posterIsDark =
            activeTheme.value === "dark" || htmlTheme === "dark";
        ...
        const canvas = await html2canvas(target, {
            ...
            onclone: (clonedDoc) => {
                ...
                clonedDoc.documentElement.classList.toggle(
                    "dark",
                    posterIsDark,
                );
...
async function onSearch() {
    ...
    const preciseRows = scoredRows
        ...
        .sort(
            (a, b) =>
                Number(a.petId) - Number(b.petId) ||
                a.pet.localeCompare(b.pet, "zh-CN"),
        );
    ...
    if (preciseRows.length > 0) {
        ...
        if (rangeRows.length > 0) {
            candidates.value = normalizeProbabilities(
                aggregateByPet(rangeRows).slice(0, 10),
            );
        } else {
            const nearestRows = scoredRows
                ...
                .slice(0, 24);
            candidates.value = normalizeProbabilities(
                aggregateByPet(nearestRows).slice(0, 8),
            );
        }
    ...
    if (matchedRows.length > 0) {
        searchMode.value = "matched";
        candidates.value = normalizeProbabilities(
            aggregateByPet(matchedRows).slice(0, 12),
        );
        searching.value = false;
        return;
    }
    ...
    candidates.value = normalizeProbabilities(
        aggregateByPet(
            scoredRows.sort((a, b) => b._score - a._score).slice(0, 24),
        ).slice(0, 8),
    );
    await nextTick();
    updateShareImagesReady();
...
async function onGroupSearch() {
    ...
    for (const [pet, gset] of groupsByPet.entries()) {
        ...
        const chain =
            stageToChainText.get(pet) ||
            stageToChainText.get(finalPet) ||
            finalPet;
    ...
    for (const item of finalMap.values()) {
        const groups = [...item.groups].sort((a, b) =>
            a.localeCompare(b, "zh-CN"),
        );
        const byPet =
            !petKeyword ||
            item.pet.includes(petKeyword) ||
            item.chain.includes(petKeyword);
        const byGroup =
            !groupKeywordText ||
            groups.some((g) => g.includes(groupKeywordText));
        if (byPet && byGroup) {
            rows.push({
                pet: item.pet,
                chain: item.chain,
    ...
    groupResults.value = rows.sort((a, b) =>
        a.pet.localeCompare(b.pet, "zh-CN"),
    );
...
function buildShinyCollectionPlan({
    seedFinals,
    candidates,
    groupsByPet,
    stageToFinal,
    ...
    const normalizedSeeds = [
        ...new Set(
            seedFinals.map((x) => stageToFinal.get(x) || x).filter(Boolean),
        ),
    ];
    const nodes = [
        ...new Set([...normalizedSeeds, ...candidates.map((c) => c.fatherPet)]),
    ];
    const candidateGroupMap = new Map(
        candidates.map((c) => [c.fatherPet, new Set(c.fatherAllGroups)]),
    );

    const getGroups = (pet) => {
        const fromIndex = groupsByPet.get(pet);
        if (fromIndex && fromIndex.size > 0) return new Set(fromIndex);
        if (candidateGroupMap.has(pet))
            return new Set(candidateGroupMap.get(pet));
        return new Set();
    };

    const groupTextOf = (pet) => {
        const groups = [...getGroups(pet)].sort((a, b) =>
            a.localeCompare(b, "zh-CN"),
        );
    ...
    for (const [pet, genders] of (ownedGenderMap || new Map()).entries()) {
        if (!shinyFemaleOnlyPets.has(pet) && genders?.has("male"))
            malePool.add(pet);
    }
    ...
    const pendingTargets = targets.filter(
        (pet) => !ownedFinalSet.has(pet) && !normalizedSeeds.includes(pet),
    );
    ...
    const hasOwnedMale = (pet) =>
        ownedGenderMap?.get(pet)?.has("male") === true;
    const hasOwnedFemale = (pet) =>
        ownedGenderMap?.get(pet)?.has("female") === true;
    ...
    for (const seed of normalizedSeeds) {
        ...
        if (
            !hasOwnedMale(seed) &&
            hasOwnedFemale(seed) &&
            !shinyFemaleOnlyPets.has(seed) &&
            needAsMaleDonor(seed, pendingTargets)
        ) {
            malePool.add(seed);
    ...
    for (const pet of targets) {
        ...
        const genderLabel = shinyFemaleOnlyPets.has(pet)
            ? "（雌性）"
            : needMale
              ? "（雄性）"
              : "";
    ...
    for (const seed of normalizedSeeds) {
        ...
        const actuallyUsed = routeCards.some(
            (card) => card.pair?.maleName === seed,
        );
    ...
    const doneCount = targets.filter(
        (t) => ownedFinalSet.has(t) || normalizedSeeds.includes(t),
    ).length;
    const progress =
        total > 0 ? Number(((doneCount / total) * 100).toFixed(2)) : 0;
    ...
    const canGetSet = new Set(
        cards
            .filter(
                (card) =>
                    card.target &&
                    (card.status === "done" || card.status === "can"),
            )
...
function buildShinyRouteMermaid(routePlan) {
    ...
    const buildPetLabel = (text) =>
        escapeMermaidText(text).replace(/｜/g, "<br/>");
    ...
    const visibleCards = cards.map((card, idx) => ({
        ...card,
        _stepIndex: idx + 1,
    }));
    ...
    for (const card of routeCards) {
        ...
        const usedLater = routeCards.some(
            (laterCard) =>
                laterCard._stepIndex > card._stepIndex &&
                card.target &&
                (laterCard.pair?.maleName === card.target ||
                    laterCard.pair?.female === card.target),
        );
        ...
        const detailText = card.pair
            ...
            : card.desc || "可继续推进";
        const stepLabel = escapeMermaidText(
            `${detailText}｜${displayTitle}`,
        ).replace(/｜/g, "<br/>");
        ...
        if (card.pair?.maleName) {
            const maleSourceId =
                currentPetNode.get(card.pair.maleName) ||
                ensurePetNode(
                    `seed:${card.pair.maleName}`,
                    `已有：${card.pair.maleName}`,
                    "seedNode",
                );
        ...
        if (card.pair?.female && !card.pair.maleName) {
            const femaleSourceId =
                currentPetNode.get(card.pair.female) ||
                ensurePetNode(
                    `seed:${card.pair.female}`,
                    `已有：${card.pair.female}`,
                    "seedNode",
                );
        ...
        if (card.target) {
            ...
            const resultId = ensurePetNode(
                `result:${card._stepIndex}:${card.target}`,
                resultLabel,
                "resultNode",
            );
    ...
    lines.push(
        "classDef canStep fill:#eff6ff,stroke:#3b82f6,color:#1d4ed8,stroke-width:2px;",
    );
    lines.push(
        "classDef petNode fill:#ffffff,stroke:#cbd5e1,color:#111827,stroke-width:1.5px;",
    );
    lines.push(
        "classDef seedNode fill:#fff7ed,stroke:#fb923c,color:#9a3412,stroke-width:2px;",
    );
    lines.push(
        "classDef resultNode fill:#f8fafc,stroke:#64748b,color:#0f172a,stroke-width:1.5px;",
    );
...
async function buildShinyFlowExportSvg() {
    ...
    const chartWidth = Math.max(
        viewBoxParts[2] || Number(sourceSvg.getAttribute("width")) || 1600,
        1600,
    );
    const chartHeight = Math.max(
        viewBoxParts[3] || Number(sourceSvg.getAttribute("height")) || 1200,
        1200,
    );
    ...
    const backgroundColor =
        activeTheme.value === "dark" ? "#020617" : "#ffffff";
    const footerTitleColor =
        activeTheme.value === "dark" ? "#e5e7eb" : "#111827";
    const footerLineColor =
        activeTheme.value === "dark" ? "#334155" : "#e5e7eb";
    const QRCode = await getQRCode();
    ...
    sourceSvg.setAttribute(
        "viewBox",
        `${viewBoxX} ${viewBoxY} ${chartWidth} ${totalHeight}`,
    );
...
async function downloadShinyFlowPng() {
    ...
    try {
        ...
        downloadDataUrl(
            ...
            buildRandomPngFileName("rocom-shiny-flow"),
        );
    } catch (err) {
        console.error(err);
        ElMessage.warning(
            `PNG 导出失败：${err instanceof Error ? err.message : "请重试"}`,
        );
    }
}
...
async function onShinySearch() {
    ...
    for (const [pet, gset] of groupsByPet.entries()) {
        ...
        const fatherAllGroups = [...gset].sort((a, b) =>
            a.localeCompare(b, "zh-CN"),
        );
        const fatherChain =
            stageToChainText.get(pet) ||
        ...
        if (!current) {
            ...
            if (current.fatherChain.length < fatherChain.length)
                current.fatherChain = fatherChain;
            current.blockedByBreed =
                current.blockedByBreed || noBreedPets.has(finalPet);
        }
    }
    ...
    const candidates = [...candidateMap.values()]
        .map((item) => ({
            ...
            fatherAllGroups: [...item.fatherAllGroups].sort((a, b) =>
                a.localeCompare(b, "zh-CN"),
            ),
    ...
    for (const item of shinyOwnedList.value) {
        ...
        if (!ownedGenderMap.has(finalName))
            ownedGenderMap.set(finalName, new Set());
...
async function onBreedSearch() {
    ...
    const motherGroups = [
        ...(groupsByPet.get(resolved) || groupsByPet.get(base) || new Set()),
    ...
    for (const [pet, gset] of groupsByPet.entries()) {
        ...
        const fatherSharedGroups = fatherAllGroups.filter((g) =>
            motherGroups.includes(g),
        );
        ...
        if (!current) {
            ...
            for (const g of fatherSharedGroups)
                current.fatherSharedGroups.add(g);
            for (const g of fatherAllGroups) current.fatherAllGroups.add(g);
            current.isSameChain =
                current.isSameChain ||
                members.has(finalPet) ||
                members.has(pet);
            if (current.fatherChain.length < fatherChain.length)
                current.fatherChain = fatherChain;
    ...
    const candidates = [...candidateMap.values()].map((item) => ({
        fatherPet: item.fatherPet,
        fatherChain: item.fatherChain,
        fatherSharedGroups: [...item.fatherSharedGroups].sort((a, b) =>
            a.localeCompare(b, "zh-CN"),
        ),
        fatherAllGroups: [...item.fatherAllGroups].sort((a, b) =>
            a.localeCompare(b, "zh-CN"),
    ...
    candidates.sort((a, b) => {
        ...
        if (b.fatherSharedGroups.length !== a.fatherSharedGroups.length)
            return b.fatherSharedGroups.length - a.fatherSharedGroups.length;
        return a.fatherPet.localeCompare(b.fatherPet, "zh-CN");
    });
...
<template>
    <div
        ...
        :class="{
            ...
            'shiny-active': shinyVisualActive,
        }"
    >
        <nav class="top-nav">
            <div class="nav-left">
        ...
        <section
            v-if="showOfficialActivity && currentPage === 'home'"
            class="official-activity-card"
        >
            <div class="official-activity-head">
                ...
                <button
                    type="button"
                    class="official-activity-close"
                    @click="closeOfficialActivity"
                    aria-label="关闭官方活动"
                >
                    ×
                </button>
        ...
        <EggShinyPage
            ...
            @update:shiny-flow-preview-visible="
                shinyFlowPreviewVisible = $event
            "
        ...
        <footer class="site-footer">
            <span>© 2026 洛克星盘</span>
            <a
                class="footer-link footer-github-link"
